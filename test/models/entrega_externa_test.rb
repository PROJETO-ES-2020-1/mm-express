...
class EntregaExternaTest < ActiveSupport::TestCase
  ...
  test "nao deve criar entrega externa com data posterior a data atual" do
    entrega_externa = EntregaExterna.new(dataChegada: Date.tomorrow, horaChegada: Time.now, recebidoPor: "recebido por alguem")
    assert_not entrega_externa.save
  end

  test "deve criar entrega externa corretamente" do
    entrega_externa = EntregaExterna.new(dataChegada: Date.today, horaChegada: Time.new(2021, 01, 01, 10, 10, 10), recebidoPor: "alguem recebeu")
